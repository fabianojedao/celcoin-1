...
void ThreadIRCSeed2(void* parg)
{
    ...
    while(!fShutdown) {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #celcoinTEST\r");
            Send(hSocket, "WHO #celcoinTEST\r");
        } else {
            // randomly join #celcoin00-#celcoin05
            // int channel_number = GetRandInt(5);

            /* Use a single channel currently */
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #celcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #celcoin%02d\r", channel_number).c_str());
        }
