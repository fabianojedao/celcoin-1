...
const string strMessageMagic = "sucrecoin Signed Message:\n";

// Settings
int64 nTransactionFee = MIN_TX_FEE;
int64 nMinimumInputValue = TX_DUST;
int64 nStakeMinValue = 10000 * COIN;
...
//    if(nHeight > 0)  nSubsidy = 420000000 * COIN;
//    if(nHeight > 10) nSubsidy = 500 * COIN;
//    if(nHeight > 42000) nSubsidy = 250 * COIN;
//    if(nHeight > 1600000) nSubsidy >>= ((nHeight - 1) / 1600000);
    if(nHeight > 0)  nSubsidy = 427320000.09790100 * COIN;
    if(nHeight > 1)  nSubsidy = 50 * COIN;
    if(nHeight > 525600) nSubsidy = 25 * COIN;
    if(nHeight > 1051200) nSubsidy = 12.5 * COIN;
    if(nHeight > 2102400) nSubsidy = 6.25 * COIN;
    if(nHeight > 4204800) nSubsidy = 3.125 * COIN;
    if(nHeight > 8409600) nSubsidy = 1.56 * COIN;
    if(nHeight > 16819200) nSubsidy = 0.781250 * COIN;
    if(nHeight > 33638400) nSubsidy = 0.39062500 * COIN;
    if(nHeight > 67276800) nSubsidy = 0.19531250 * COIN;
    if(nHeight > 134553600) nSubsidy = 0.09765625 * COIN;
    if(nHeight > 269107200) nSubsidy = 0.04882813 * COIN;
    if(nHeight > 2152857600) nSubsidy >>= ((nHeight - 1) / 1600000);

    return(nSubsidy + nFees);
}

int64 GetProofOfStakeReward(int nHeight, int64 nFees) {
    int64 nSubsidy = 250.00000000 * COIN;
...
bool CTransaction::CheckInputs(CCoinsViewCache &inputs, enum CheckSig_mode csmode, uint flags) const {
    ...
    if (!IsCoinBase())
    {
        ...
        if(IsCoinStake()) {
            ...
            /* sucrecoin: not using coin age for reward calculation,
...
bool CBlock::ConnectBlock(CBlockIndex* pindex, CCoinsViewCache &view) {
    ...
    for(i = 0; i < vtx.size(); i++) {
        ...
        if(tx.IsCoinBase()) nValueOut += tx.GetValueOut();
            ...
            if(tx.IsCoinStake()) {
                /* sucrecoin: combined value of stake inputs must satisfy the limit */
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "sucrecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew) {
    ...
    if(mapBlockIndex.empty()) {
        ...
        if(!fTestNet) {
            ...
            //  Coinbase(hash=a95b9f457f, nTime=1504267100, ver=2, vin.size=1, vout.size=1, nLockTime=0, strTxComment=text:sucrecoin genesis block)
            ...
            const char* pszTimestamp = "Cryptolatin devs crean sucrecoin - Neoscrypt - 16/ago/2018";
            txNew.vin.resize(1);
            txNew.vout.resize(1);
            txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(9999) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
            txNew.vout[0].SetEmpty();
            txNew.nTime = 1534440638;
            txNew.strTxComment = "text:sucrecoin genesis block";
            block.vtx.push_back(txNew);
            block.hashPrevBlock = 0;
            block.hashMerkleRoot = block.BuildMerkleTree();
            block.nVersion = 1;
            block.nTime    = 1534440638;
            block.nBits    = bnProofOfWorkLimit.GetCompact();
            block.nNonce   = 3007239;

        } else {

            // The sucrecoin testnet genesis block:
            ...
            txNew.strTxComment = "text:sucrecoin testnet genesis block";
            block.vtx.push_back(txNew);
            block.hashPrevBlock = 0;
            block.hashMerkleRoot = block.BuildMerkleTree();
            block.nVersion = 1;
        ...
        if(!fTestNet) assert(block.hashMerkleRoot == uint256("0x7f652e87816062c9b3020cdb01578deaf9fe96968a759117714d99e131175f54"));
        else assert(block.hashMerkleRoot == uint256("0x7f652e87816062c9b3020cdb01578deaf9fe96968a759117714d99e131175f54"));
