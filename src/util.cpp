...
#else
    /* Linux, Mac OS X, *BSD and so on: ~/.celcoin */
    char* pszHome = getenv("HOME");
    if(!pszHome || !strlen(pszHome))
      /* Must be root if no $HOME set */
#if (__APPLE__)
      path = fs::path("/private/var/root/.celcoin");
#else
      path = fs::path("/root/.celcoin");
#endif
    else
      path = fs::path(pszHome) / ".celcoin";
#endif
...
boost::filesystem::path GetConfigFile()
{
    ...
    else pathConfigFile = fs::path("celcoin.conf");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "celcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong CelCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("CelCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
